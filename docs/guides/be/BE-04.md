...
│  ... hasta Capítulo 6                                                       │
│                                                                              │
│  TOTAL: 64 objetivos de aprendizaje (Learning Objectives)                   │
...
> [!NOTE]
> **Validación de `text`:** usa `min_length=1`, no `min_length=10`. En PDFs reales, la extracción puede generar fragmentos válidos muy cortos; esos casos deben llegar al JSON y evaluarse con `warnings`/`is_complete`, no romper la respuesta Pydantic.
...
```text
✅ Test 1 PASÓ: Se detectaron tópicos (64 encontrados)
✅ Test 2 PASÓ: Distribución K correcta (K1=14, K2=42, K3=8)
...
📊 Resumen de detección:
   Tópicos detectados: 64
   Distribución K: {'K1': 14, 'K2': 42, 'K3': 8}
   Completo: True
```
...
| `Test 6 FALLÓ: La detección no es completa` | El fixture no tiene todos los 64 tópicos esperados, o el `completeness_threshold` es muy alto | Verifica que `SAMPLE_SYLLABUS_TEXT` contiene los 64 tópicos listados en `EXPECTED_TOPIC_CODES`. Cada uno debe tener el formato correcto. |
...
2. ✅ Creaste el fixture `sample_syllabus_text.py` con una muestra representativa del syllabus ISTQB CTFL v4.0.1 que incluye los 64 tópicos con sus niveles K.
