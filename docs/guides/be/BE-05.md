...
2. **BE-04** — Un `TopicDetectorService` que toma el texto crudo del PDF y detecta los 64 objetivos de aprendizaje (FL-x.x.x) del syllabus ISTQB CTFL v4.0.1. Extrae el nivel K (K1/K2/K3), nombre, y texto de cada tópico.
...
Ejemplo real del ISTQB CTFL v4.0.1: `(14 × 0.5) + (42 × 1.0) + (8 × 1.5) = 7.0 + 42.0 + 12.0 = 61.0 horas`
...
> **Nota sobre `TopicInfo.text`:** usamos `min_length=1` en vez de `min_length=10` porque un PDF real puede producir fragmentos cortos, por ejemplo `"riesgo"`. El endpoint debe responder JSON y reportar advertencias, no fallar con `500` por texto no vacío.
...
cuando se procesa el fixture de texto del syllabus ISTQB CTFL v4.0.1.
...
- ✅ Archivo `backend/tests/test_extractor.py` creado con 9 tests de verificación.
...
**Salida esperada:**
```
✅ Test 1 PASÓ: Pipeline produce resultado válido (64 tópicos en diccionario)
✅ Test 2 PASÓ: Diccionario tiene estructura correcta
✅ Test 3 PASÓ: Horas estimadas correctas (61.0h)
✅ Test 4 PASÓ: Snapshot de tópicos coincide (64 tópicos, K1=14, K2=42, K3=8)
...
📊 Resumen del ExtractorService:
   Tópicos en diccionario: 64
   Distribución K: {'K1': 14, 'K2': 42, 'K3': 8}
   Horas estimadas: 61.0
   Completo: True
...
| 5 | `backend/tests/test_extractor.py` creado | `python -m tests.test_extractor` → 9/9 pasan |
...
| `500 Internal Server Error` al probar `/extract-pdf-full` con PDF real | `TopicInfo.text` rechaza fragmentos cortos si quedó configurado como `min_length=10` | Usa `min_length=1`: el backend debe aceptar texto corto no vacío y reportar problemas mediante `warnings` |
