...
- **$5/mes** con el plan `basic-xxs` (cubierto por los créditos gratuitos de DO).
...
Si DO te pregunta por un **nombre** del servicio, usa: `api`.

Haz click en **"Next"**.

#### D.5 — Seleccionar plan

1. **Plan:** Selecciona **"Basic"**.
2. **Instance Size:** Selecciona **"$5.00/mo — Basic (XXS)"**:
   - 1 vCPU compartida
   - 256 MB RAM
...
En este paso, DO te muestra las variables definidas en app.yaml. Puedes verificar que aparecen:
...
> ⚠️ **No configures los secrets todavía.** Los configuraremos correctamente en el Paso E, después del primer deploy. Por ahora, los valores placeholder (`CONFIGURAR_EN_DASHBOARD`) están bien porque el backend no los usa activamente en BE-06.

Haz click en **"Next"**.

#### D.7 — Revisar y crear

1. **Region:** Verifica que es `New York` (nyc) o la que hayas elegido.
2. **App Name:** `istqb-study-agent-api` (o el que prefieras).
3. Revisa el resumen de la configuración.
...
**Acción:** Configurar los valores reales de las variables SECRET en el dashboard.

1. En el dashboard de tu app en DigitalOcean, ve a la pestaña **"Settings"**.
2. Haz scroll hasta la sección **"App-Level Environment Variables"** (o click en el componente `api` y luego en "Environment Variables").
3. Busca las variables `SUPABASE_URL` y `SUPABASE_SERVICE_ROLE_KEY`.
4. Haz click en el ícono de editar (lápiz) junto a cada una.
5. Introduce los valores reales:
   - `SUPABASE_URL`: La URL de tu proyecto de Supabase (ej: `https://xxxxxxxxxxxx.supabase.co`).
     - Referencia: valor de `NEXT_PUBLIC_SUPABASE_URL` en tu `.env.local` (creado en **DB-01, Paso B**).
   - `SUPABASE_SERVICE_ROLE_KEY`: La clave `service_role` de tu proyecto de Supabase.
     - Referencia: valor de `SUPABASE_SERVICE_ROLE_KEY` en tu `.env.local` (creado en **DB-01, Paso B**).
6. Haz click en **"Save"**.

> ⚠️ **Esto disparará un re-deploy automático** porque las variables de entorno cambiaron. Espera a que el deploy termine.
...
1. En el dashboard de tu app, en la parte superior verás la URL generada automáticamente:
   ...
   https://istqb-study-agent-api-xxxxx.ondigitalocean.app
   ```
   (donde `xxxxx` es un hash aleatorio que DO agrega).
...
```powershell
# Desde PowerShell
Invoke-RestMethod -Uri "https://istqb-study-agent-api-xxxxx.ondigitalocean.app/health"
```
...
```
https://istqb-study-agent-api-xxxxx.ondigitalocean.app/docs
```
...
1. Abre `https://tu-url.ondigitalocean.app/docs`
...
```powershell
# Ajusta las rutas y URLs
$pdfPath = "C:\ruta\al\ISTQB_CTFL_v4.0.pdf"
$apiUrl = "https://istqb-study-agent-api-xxxxx.ondigitalocean.app"
...
> ⚠️ **Si la respuesta tarda más de 30 segundos** o falla con timeout, el plan `basic-xxs` (256 MB RAM) puede ser insuficiente para procesar el PDF completo. Considera escalar a `basic-xs` (512 MB) temporalmente desde Settings → Resources en el dashboard de DO.
...
3. Selecciona el componente **"api"** si hay múltiples componentes.
...
```powershell
# Reemplaza con tu URL real de DigitalOcean
$apiUrl = "https://istqb-study-agent-api-xxxxx.ondigitalocean.app"
...
| **Error 502 Bad Gateway** | El contenedor tardó demasiado en arrancar o se quedó sin memoria | Verificar los Runtime Logs. Si el problema es memoria, escalar a `basic-xs` (512 MB) desde Settings → Resources |
...
| **`POST /extract-pdf-full` timeout** | El plan `basic-xxs` (256 MB) no tiene suficiente memoria para el PDF | Escalar a `basic-xs` (512 MB, $10/mes) desde Settings → Resources. El procesamiento del PDF puede necesitar más RAM |
| **Variables de entorno no se leen** | Las variables no están configuradas en el componente correcto | En DO dashboard, verificar que las vars están en el componente `api`, no como "App-Level" vs "Component-Level" |
