...
## Addendum Correctivo Obligatorio: endurecer `handle_new_user()`

**Causa raíz:** la versión histórica de `public.handle_new_user()` era `SECURITY DEFINER`, pero no fijaba un `search_path` seguro y conservaba permisos de ejecución para roles que nunca deben invocar directamente una función de trigger. Supabase Advisors reportaba `function_search_path_mutable` y ejecución insegura para `anon` y `authenticated`.

La corrección autoritativa está en:

`supabase/migrations/20260713005928_harden_handle_new_user.sql`

El contrato vigente:

1. fija `SET search_path = ''`;
2. califica `public.user_profiles` explícitamente dentro de la función;
3. revoca `EXECUTE` a `PUBLIC`, `anon` y `authenticated`;
4. concede `EXECUTE` únicamente a `supabase_auth_admin`, rol interno que ejecuta el trigger de Auth.

### Resultado verificado (12/07/2026)

- La migración está aplicada y el historial local/remoto está sincronizado.
- Un registro temporal creó automáticamente la fila esperada en `public.user_profiles` con su `full_name`.
- El usuario y su fixture se eliminaron después de la prueba.
- Supabase Advisors dejó de reportar los tres warnings asociados a la función.

**Gate de salida:** satisfecho. DB-05 permanece `Implementado y verificado` con el contrato endurecido.
