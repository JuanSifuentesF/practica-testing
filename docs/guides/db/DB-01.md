...
> [!IMPORTANT]
> **REGLA DE SEGURIDAD Y ESTABILIDAD:**  
> Hoy en día, **ya no se recomienda instalar de forma global con `npm install -g supabase`**. Las instalaciones globales de npm pueden generar conflictos de versiones, fallos de permisos en Windows (errores de permisos de administrador) y potenciales vulnerabilidades de seguridad si se ejecutan scripts maliciosos con privilegios del sistema.
>
> En su lugar, el estándar profesional utiliza una de las siguientes dos opciones seguras:

#### Opción 1: Usando Scoop (La vía recomendada para instalación global nativa en Windows)
[Scoop](https://scoop.sh) es el instalador de paquetes de línea de comandos preferido en Windows. Descarga directamente el ejecutable compilado en Go de Supabase, lo aísla en tu usuario (sin requerir permisos de administrador) y evita intermediarios de Node.js.

1. Abre tu terminal de **PowerShell** y verifica si tienes Scoop instalado:
   ```powershell
   scoop --version
   ```
2. Si no lo tienes, instálalo ejecutando (asegúrate de no estar en modo Administrador):
   ```powershell
   iwr -useb get.scoop.sh | iex
   ```
3. Agrega el repositorio ("bucket") oficial de Supabase e instala la CLI:
   ```powershell
   scoop bucket add supabase https://github.com/supabase/scoop-bucket.git
   scoop install supabase
   ```

#### Opción 2: Usar `npx` sin instalación global (La vía recomendada para monorepos)
Si prefieres no instalar un manejador de paquetes de Windows, puedes usar **`npx`** (incluido con Node.js/npm). Esto descarga y ejecuta la CLI en memoria al instante.

Puedes ejecutar cualquier comando agregando `npx` por delante de `supabase`. Por ejemplo, en lugar de `supabase init`, usarías:
```powershell
npx supabase init
```
*(Esta opción es extremadamente limpia, garantiza que siempre uses la última versión oficial de la CLI y elimina cualquier riesgo de seguridad de paquetes globales).*
...
> [!TIP]
> **Si estás usando la Opción 2 (`npx`):**  
> Recuerda anteponer siempre **`npx `** antes de cualquier comando `supabase` (ej. `npx supabase init`, `npx supabase login`, `npx supabase link...`). Si usaste Scoop (Opción 1), puedes correrlos directamente como `supabase`.
...
3. Inicia sesión en tu cuenta remota desde la terminal:
   ...
   supabase login
   ```
   *(Si usas npx: `npx supabase login`)*
...
4. Enlaza tu repositorio local a tu base de datos remota:
   ...
   ```
   *(Si usas npx: `npx supabase link --project-ref [ID-DE-TU-PROYECTO]`)*
   *Reemplaza `[ID-DE-TU-PROYECTO]` por la cadena alfanumérica que se encuentra en la URL de tu panel de control de Supabase (ej. si la URL es `https://supabase.com/dashboard/project/xyz123abc`, el project-ref is `xyz123abc`). La terminal te solicitará la contraseña (Database Password) que definiste en el Paso A.*
...
supabase status
```
*(Si usas npx: `npx supabase status`)*
