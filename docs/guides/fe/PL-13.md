...
**Dependencias verificadas:** DA-01 y PL-09 conservan su cierre. PL-12 también
cerró 7/7 con su contrato HTTP reconciliado; no cambia el entregable de dashboard.
        ...
        <>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {(Object.keys(TYPE_META) as PracticeExerciseType[]).map((type) => (
              ...
              >
                {TYPE_META[type].icon} {TYPE_META[type].label}: {stats.by_type[type]}
              </span>
            ))}
          </div>
          {most && (
...
### Evidencia de cierre (12/07/2026)

- `npx tsc --noEmit --project frontend/tsconfig.json`: finalizó con código `0`.
- `npm --prefix frontend run build`: compiló correctamente con Next.js `16.2.9`, ejecutó TypeScript y generó las 21 páginas.
- Warning no bloqueante: la convención `middleware` está deprecada en Next.js 16 y se migrará a `proxy.ts` en PL-14. PL-13 no modifica autenticación ni routing global.
...
1. El DTO unico del dashboard ya incluye `practice_stats`.
2. Las metricas usan ejercicios y submissions mediante queries planas compatibles con `Relationships: []`.
3. Los cuatro valores de `PracticeExerciseType` se conservan sin aliases.
4. La tarjeta es presentacional y no agrega otro fetch.
5. Fixtures, respuesta API, estados vacios y responsive quedaron verificados.

La evidencia puede revalidarse con:

```text
Valida mi progreso de la guia PL-13 en su seccion 6
```

**PL-14 ya está implementada y verificada.** El trabajo vigente está en el gate
correctivo pre-QA del roadmap v2.13; no vuelvas a generar PL-14.
