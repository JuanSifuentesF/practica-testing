...
**Estado:** guia generada; implementacion pendiente.
...
- [ ] `PracticeStats` usa cuatro tipos canonicos y se reexporta.
- [ ] `practice-stats.ts` tiene agregador puro y fixtures offline.
- [ ] DA-01 consulta ejercicios por usuario/documento y submissions por IDs, sin joins.
- [ ] Ruta conserva auth, RLS, runtime, cache y wrapper existentes.
- [ ] `PracticeProgressCard` es presentacional y no hace fetch.
- [ ] Dashboard integra la tarjeta con el fetch existente.
- [ ] `tsc` y build pasan.
