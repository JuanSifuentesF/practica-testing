...
> **Fecha:** 29 junio 2026  
> **Addendum correctivo vigente (20/07/2026):** la autoridad vive en
> `apply_session_adaptation_v2`; score, action, método y tópicos salen de DB.
> El marcador privado hace replay exactly-once y `finalize_quiz_and_adapt_claimed`
> une grading y adaptación. El endpoint legacy conserva solo una firma puente.
...
## Sección 5: 🛠️ Implementación Paso a Paso — Histórica, Sustituida

> **NO COPIAR ESTA SECCIÓN.** Conserva la implementación original como registro
> pedagógico, pero sus escrituras separadas y su `next_method` enviado por el
> cliente fueron sustituidos por `apply_session_adaptation_v2` y
> `finalize_quiz_and_adapt_claimed` en la migración `20260719224046`. El contrato
> ejecutable vigente está en el código real y en el addendum del encabezado.
...
- [x] ADVANCE → `topic_progress.status = 'mastered'`
- [x] REINFORCE → 1 sesión `reinforcement` en DB
- [x] RESTRUCTURE → 2 sesiones + `estimated_end_date` extendido
