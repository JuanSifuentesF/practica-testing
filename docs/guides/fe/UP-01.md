...
- [ ] Archivo `lib/format.ts` creado con las funciones `formatFileSize`, `MAX_FILE_SIZE`, `ACCEPTED_FILE_TYPES` y `ACCEPTED_EXTENSIONS`
...
- [ ] Archivo `app/(dashboard)/setup/_components/file-preview.tsx` creado
- [ ] El componente exporta `FilePreview` con props tipadas (`FilePreviewProps`)
- [ ] Usa la utilidad `formatFileSize` de `lib/format.ts`
...
- [ ] Archivo `app/(dashboard)/setup/_components/pdf-dropzone.tsx` creado
- [ ] Soporta drag & drop Y click para seleccionar archivo
- [ ] Valida tipo MIME (solo PDF) y tamaño (máximo 20MB)
- [ ] Cambia de estilo visual cuando se arrastra un archivo encima
- [ ] Muestra mensaje de error si la validación falla
- [ ] Es accesible por teclado (Enter/Space)
...
- [ ] Archivo `app/(dashboard)/setup/_components/study-config.tsx` creado
- [ ] Exporta `StudyConfig`, `StudyConfigData` (interfaz) y `DEFAULT_CONFIG`
- [ ] Input de días con validación min=1, max=30
- [ ] Selects de horarios con opciones predefinidas
- [ ] Resumen calculado que se actualiza en tiempo real
...
- [ ] Archivo `app/(dashboard)/setup/page.tsx` creado
- [ ] Renderiza `PdfDropzone`, `FilePreview`, `StudyConfig` y el botón de submit
- [ ] El estado global del formulario se maneja con `useState`
- [ ] El botón se habilita solo cuando hay un archivo seleccionado
- [ ] El loading state muestra spinner y desahabilita todos los inputs
- [ ] El placeholder `handleSubmit` funciona (alert con los datos)
...
- [ ] Verificado que `main-nav.tsx` tiene un link a `/setup`
- [ ] Verificado que `mobile-nav.tsx` tiene un link a `/setup`
...
- [ ] `lib/format.ts` — utilidad `formatFileSize()` + constantes
- [ ] `app/(dashboard)/setup/page.tsx` — página principal de setup
- [ ] `app/(dashboard)/setup/_components/pdf-dropzone.tsx` — zona drag & drop
- [ ] `app/(dashboard)/setup/_components/study-config.tsx` — formulario configuración
- [ ] `app/(dashboard)/setup/_components/file-preview.tsx` — preview del archivo
- [ ] `components/ui/card.tsx` — componente shadcn/ui instalado
- [ ] `components/ui/separator.tsx` — componente shadcn/ui instalado
- [ ] `npx tsc --noEmit` — sin errores de TypeScript
- [ ] Zona de drop funciona con drag & drop Y click
- [ ] Validación rechaza archivos no-PDF y archivos >20MB
- [ ] Cambiar configuración actualiza el resumen en tiempo real
- [ ] Loading state funciona correctamente (spinner + disabled)
- [ ] Responsive a 375px — todo visible y usable
