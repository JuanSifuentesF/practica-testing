...
> **Fecha:** 30 junio 2026  
> **Addendum correctivo vigente (20/07/2026):** el resultado persistido se
> valida con un parser runtime estricto antes de llegar al navegador. Un replay
> rehidrata evaluación/adaptación, detiene el timer y refresca métricas del plan;
> campos adicionales o acciones divergentes fallan cerrados.
...
> **NO COPIAR ESTA SECCIÓN.** Los snippets originales mantienen `correct` y
> `explanation` en `QuizQuestion` cliente y separan `/evaluate` de `/adapt`.
> Fueron sustituidos por el resultado autoritativo `question_results`, el body
> mínimo `{ attempt_id, answers }` y la finalización/adaptación atómica claimed.
> Usa el código real y el addendum del encabezado.
