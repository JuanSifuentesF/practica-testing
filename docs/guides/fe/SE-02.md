...
> **Guía siguiente:** SE-03 — UI: TheoryPanel (45 min, JSON estructurado, timer)
> **Fecha:** Junio 2026
...
En **SE-03 — UI: TheoryPanel (45 min, JSON estructurado, timer)**, construiremos:
...
- **Renderizado del JSON estructurado** (`TheoryContent.topics[]`) usando JSX tipado, sin depender de `react-markdown`.
...
- **Botón "Listo, ir al quiz"** que navega al placeholder de quiz (`phase=quiz`) hasta SE-04/SE-05.
...
### ✅ Verificación funcional (29 junio 2026)

Validación con `istqb-step-validator` + pruebas runtime en navegador:

| Verificación | Resultado |
|---|---|
| `tsc --noEmit` | ✅ 0 errores |
| 4/4 archivos entregables existen | ✅ |
| `types/theory.ts`: 6 interfaces con `MethodUsed` tipado | ✅ |
| `types/index.ts`: 6 tipos re-exportados de `./theory` | ✅ |
| `lib/prompts/theory.ts`: 2 funciones con 3 variantes de método | ✅ |
| `POST /api/sessions/[id]/theory` con Gemini 2.5 Flash | ✅ 5 tópicos generados |
| Segunda llamada retorna `cached: true` (idempotencia) | ✅ mismo timestamp |
| `force: true` regenera contenido | ✅ nuevo timestamp `20:02:31` |
| `response_format: json_object` + `temperature: 0.7` | ✅ |
| `AbortController` con timeout `504` | ✅ |
| Validación estricta (falla 502 si teoría incompleta) | ✅ |
| `status` cambia de `pending` a `active` + `started_at` | ✅ |
| Gemini default / `gpt-4o-mini` fallback | ✅ |

---

## Addendum Correctivo: Cascada Compartida de Teoría

`POST /api/sessions/[id]/theory` usa `frontend/lib/ai/model-cascade.ts`. Prueba `GEMINI_THEORY_MODEL` si existe, después los candidatos Gemini compartidos y finalmente `OPENAI_THEORY_MODEL`/`gpt-4o-mini`. Un candidato se descarta por error de proveedor, timeout, JSON inválido o teoría que no supera `validateTheoryTopics()`. Sólo persiste la primera teoría válida; todos los timeouts devuelven `504` y los demás fallos totales devuelven `502`.

Verifica con `Select-String -LiteralPath "frontend\app\api\sessions\[id]\theory\route.ts" -Pattern "createModelRuntimes|GEMINI_THEORY_MODEL"` sin imprimir claves.
