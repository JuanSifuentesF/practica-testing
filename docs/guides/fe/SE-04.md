...
> **Addendum correctivo vigente (20/07/2026):** el quiz completo se persiste en
> `private`; el cliente recibe solo pregunta/opciones. La generación usa un
> lease cercado, exige 10-12 preguntas, máximo 12 tópicos y cobertura de cada
> código validado contra `documents.topics_json`. Snippets anteriores que
> devuelvan `correct` o `explanation` antes de evaluar son históricos.
> Settings conserva la autoridad de proveedor/modelo; el fallback no cruza de
> proveedor y `AI_INVALID_RESPONSE` termina sin otra llamada facturable.
...
## Addendum Correctivo: Cascada Compartida de Quiz

`POST /api/sessions/[id]/quiz` usa `frontend/lib/ai/model-cascade.ts`. Prueba `GEMINI_QUIZ_MODEL`, los candidatos Gemini compartidos y luego `OPENAI_QUIZ_MODEL`/`gpt-4o-mini`. Si una respuesta no se puede parsear o no supera `validateQuizQuestions()`, no entra al cache y la ruta intenta el siguiente modelo. El cache conserva únicamente quizzes válidos.

La lista de candidatos y las claves permanecen server-only. Los logs sólo registran modelo, estado y métricas, nunca contenido crudo o secretos.
