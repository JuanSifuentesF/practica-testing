...
3. ✅ **Creaste `app/api/plan/generate/route.ts`** — Route Handler completo con `runtime = "nodejs"`, `parseBody()`, autenticación → validación → fetch tópicos → ownership check → llamada a OpenAI → validación del plan → respuesta.

4. ✅ **Modificaste `setup/page.tsx`** — Flujo de 3 etapas (upload → extract → generate) con barra de progreso de 4 niveles, visualización del resultado del plan y redirect final a `/plan?document_id=...`.

5. ✅ **Validación defensiva** — El plan generado se valida contra los tópicos originales: verificas que no hay codes inventados, no hay duplicados, la estructura es correcta y cualquier omisión queda declarada en `coverage.omitted_topic_codes`.
...
UP-05 conectará lo que generamos aquí (el JSON del plan) con la base de datos real, completando el puente entre la IA y los datos persistentes.

---

*Guía UP-04 — Junio 2026*  
