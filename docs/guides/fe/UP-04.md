...
- [x] `frontend/lib/openai.ts` creado — exporta `openai` y `OPENAI_MODEL`
- [x] `frontend/app/api/plan/generate/route.ts` creado — Route Handler completo con `runtime = "nodejs"`
- [x] `parseBody()` valida el body antes de usarlo
- [x] El prompt usa `MAX_TOPICS_IN_PROMPT` y `text_preview` para controlar tokens
- [x] El schema incluye `coverage.covered_topic_codes` y `coverage.omitted_topic_codes`
- [x] `frontend/app/(dashboard)/setup/page.tsx` modificado — 3 etapas en el flujo
- [x] El flujo Upload → Extract → Generate Plan funciona end-to-end en localhost
- [x] La barra de progreso muestra las 4 etapas correctamente
- [x] El resultado del plan muestra: sesiones, fecha inicio, fecha fin, modelo
- [x] El éxito redirige a `/plan?document_id=...` sin reactivar el botón antes de navegar
- [x] Logs del servidor muestran confirmación exitosa
- [x] Si OpenAI falla → error descriptivo al usuario (no pantalla blanca)
- [x] Cambiar `OPENAI_MODEL` en `.env.local` no requiere cambios de código
