...
Estas variables **ya deben existir** en tu `.env.local` (creadas en FE-01 y DB-01):

```powershell
# Verificar que las variables existen (sin mostrar valores)
cd c:\Users\jsife\OneDrive\Desktop\Repositorios\practica-testing\frontend
Select-String -Pattern "OPENAI_API_KEY" .env.local
# Esperado: OPENAI_API_KEY=sk-proj-...

Select-String -Pattern "OPENAI_MODEL" .env.local
# Esperado: OPENAI_MODEL=gpt-4o-mini
```

| Variable | Origen | ¿Dónde se usa? |
|---|---|---|
| `OPENAI_API_KEY` | Tu cuenta de OpenAI | `lib/openai.ts` (servidor) |
| `OPENAI_MODEL` | `.env.local` | `lib/openai.ts` (servidor) |
...
### Paso A — Crear `lib/openai.ts` (Cliente OpenAI Configurado)

**Archivo:** `frontend/lib/openai.ts`  
**Tipo:** Módulo del servidor (NO es un componente React)

Este módulo crea y exporta una instancia configurada del cliente de OpenAI. Al centralizar la configuración en un solo archivo, evitamos duplicar la lógica de conexión en cada Route Handler que necesite usar OpenAI.
...
Crea el archivo `frontend/lib/openai.ts` con el siguiente contenido:
...
- [ ] Archivo `frontend/lib/openai.ts` creado
- [ ] Exporta `openai` (instancia del cliente) y `OPENAI_MODEL` (string del modelo)

---

### Paso B — Diseñar el Prompt de Generación de Plan
...
- [ ] Entiendes la estructura system/user del prompt
- [ ] Entiendes el schema JSON que OpenAI debe retornar
- [ ] El prompt está documentado (no es código aún, se implementa en el Paso C)

---

### Paso C — Crear `app/api/plan/generate/route.ts` (API Route)

**Archivo:** `frontend/app/api/plan/generate/route.ts`  
**Tipo:** Route Handler (servidor, sin "use client")

Este es el archivo principal de UP-04. Sigue el mismo patrón de fases que aprendimos en UP-03 (`extract/route.ts`): Validar → Autenticar → Leer datos → Llamar servicio externo → Validar respuesta → Retornar.

#### Código

Crea la carpeta `frontend/app/api/plan/generate/` y dentro el archivo `route.ts`:
...
- [ ] Directorio `frontend/app/api/plan/generate/` creado
- [ ] Archivo `frontend/app/api/plan/generate/route.ts` creado
- [ ] Declara `export const runtime = "nodejs"` para el SDK de OpenAI
- [ ] La ruta acepta POST con `{ document_id, config }`
- [ ] Usa `parseBody()` para validar el body en una función dedicada
- [ ] Valida autenticación, ownership, existencia de tópicos
- [ ] Construye prompt con system + user messages
- [ ] Limita el prompt con `MAX_TOPICS_IN_PROMPT` y `text_preview`
- [ ] Llama a OpenAI con JSON mode activado
- [ ] Valida la respuesta: estructura, topic_codes, duplicados y `coverage.omitted_topic_codes`
- [ ] Retorna el plan con fechas calculadas

---

### Paso D — Modificar `setup/page.tsx` para Llamar a `/api/plan/generate`

**Archivo:** `frontend/app/(dashboard)/setup/page.tsx`  
**Tipo:** Client Component (ya existente, modificación)

Actualmente, después de la extracción exitosa, la página redirige al dashboard con un `setTimeout`. Ahora necesitamos agregar una **tercera etapa**: la generación del plan y luego navegar hacia la futura vista del plan con `document_id` en la URL.
...
Aplica estos cambios al archivo `frontend/app/(dashboard)/setup/page.tsx`:
...
- [ ] `ProcessStage` incluye `"generating"`
- [ ] `STAGE_MESSAGES` tiene mensaje para `"generating"`
- [ ] Nuevo estado `planResult` con tipo `PlanResult`
- [ ] `handleSubmit` tiene 3 etapas: upload → extract → generate
- [ ] La barra de progreso muestra 4 niveles (20% → 45% → 75% → 100%)
- [ ] El resultado del plan muestra sesiones, fechas y modelo
- [ ] El error handling restaura `isLoading` a `false` solo en `catch`
- [ ] En éxito, navega a `/plan?document_id=...` y no reactiva el botón antes de navegar
...
2. **Navega a `http://localhost:3000/setup`** en tu navegador.

3. **Prueba el flujo completo:**
   - Selecciona el PDF del syllabus ISTQB
   - Configura los días (deja 7 por defecto)
   - Click en "🚀 Generar mi plan de estudio"
   - Observa las 3 etapas de progreso:
     - 20% — "Subiendo PDF al almacenamiento seguro..."
     - 45% — "Analizando el syllabus con IA..."
     - 75% — "Generando tu plan de estudio personalizado con IA..."
     - 100% — "¡Plan generado exitosamente! Redirigiendo..."

4. **Verificación visual al completar:**
   - Deberías ver "📊 63 tópicos detectados" (o el número que tenga tu PDF)
   - Deberías ver "📅 Plan generado: 14 sesiones" (si usaste 7 días)
   - Deberías ver la fecha de inicio (hoy) y fin estimado (para 7 días: hoy + 6 días)
   - Deberías ver el modelo usado (gpt-4o-mini)
   - Después de ~3 segundos, redirige a `/plan?document_id=...`

5. **Revisa los logs del servidor** en la terminal de PowerShell donde corre `npm run dev`:

```
[Plan] Generando plan para 63 tópicos, 7 días, horarios 06:00/22:00
[Plan] ✅ Plan generado exitosamente: 14 sesiones, 7 días, modelo: gpt-4o-mini, tokens: ~2500
```
...
|---|---|---|
| Sin API key | Borrar `OPENAI_API_KEY` del `.env.local`, reiniciar `npm run dev` | Error claro en la consola: "OPENAI_API_KEY no está definida" |
| Modelo inválido | Cambiar `OPENAI_MODEL=gpt-inexistente` en `.env.local` | Error 502: "Error al comunicarse con el servicio de IA" |
| Document sin tópicos | Crear un documento vacío en la DB y llamar `/api/plan/generate` con su ID | Error 422: "El documento no tiene tópicos extraídos" |
| Body inválido | Enviar POST sin config | Error 400: "Body inválido. Esperado: { document_id, config: ... }" |

### Checklist de Entregables Completo

- [ ] `frontend/lib/openai.ts` creado — exporta `openai` y `OPENAI_MODEL`
- [ ] `frontend/app/api/plan/generate/route.ts` creado — Route Handler completo con `runtime = "nodejs"`
- [ ] `parseBody()` valida el body antes de usarlo
- [ ] El prompt usa `MAX_TOPICS_IN_PROMPT` y `text_preview` para controlar tokens
- [ ] El schema incluye `coverage.covered_topic_codes` y `coverage.omitted_topic_codes`
- [ ] `frontend/app/(dashboard)/setup/page.tsx` modificado — 3 etapas en el flujo
- [ ] El flujo Upload → Extract → Generate Plan funciona end-to-end en localhost
- [ ] La barra de progreso muestra las 4 etapas correctamente
- [ ] El resultado del plan muestra: sesiones, fecha inicio, fecha fin, modelo
- [ ] El éxito redirige a `/plan?document_id=...` sin reactivar el botón antes de navegar
- [ ] Logs del servidor muestran confirmación exitosa
- [ ] Si OpenAI falla → error descriptivo al usuario (no pantalla blanca)
- [ ] Cambiar `OPENAI_MODEL` en `.env.local` no requiere cambios de código
...
|---|---|---|
| `Error: [OpenAI] OPENAI_API_KEY no está definida` | La variable `OPENAI_API_KEY` no existe en `.env.local` | Agregar `OPENAI_API_KEY=sk-proj-tu-clave` en `frontend/.env.local`. Reiniciar `npm run dev` (los cambios en `.env.local` requieren reinicio) |
| `Error 429: Rate limit` | Tu cuenta de OpenAI alcanzó el límite de peticiones por minuto | Espera 60 segundos e intenta de nuevo. Si persiste, revisa tu plan en [platform.openai.com](https://platform.openai.com) |
| `Error 502: OpenAI no generó un plan` | El modelo retornó una respuesta vacía o inválida | Intenta de nuevo. Si persiste, cambia `OPENAI_MODEL=gpt-4o` en `.env.local` (modelo más capaz) y reinicia |
| `Error al parsear JSON de OpenAI` | El modelo incluyó texto antes/después del JSON (raro con `response_format`) | Verifica que el prompt incluye la instrucción "Responde SOLO con JSON". Reintentar suele resolver |
| `Tópicos duplicados en el plan` | El modelo repitió topic_codes en múltiples sesiones | La validación detecta esto y retorna error. Reintenta — GPT genera resultados diferentes cada vez |
| `coverage.omitted_topic_codes` no coincide | El modelo omitió tópicos pero no los declaró, o declaró códigos inexistentes | La validación retorna error. Reintenta; si persiste, reduce `temperature` a `0.2` y refuerza la instrucción de coverage |
| `Edge runtime does not support...` | El Route Handler se ejecutó en Edge Runtime y el SDK de OpenAI requiere Node.js | Verifica que `route.ts` contiene `export const runtime = "nodejs"` |
| `Module not found: Can't resolve '@/lib/openai'` | El alias `@/` no apunta al directorio correcto o el archivo no existe | Verificar que `frontend/lib/openai.ts` existe. Verificar `tsconfig.json` tiene `"paths": { "@/*": ["./\*"] }` |
| `TypeError: openai.chat.completions.create is not a function` | Versión incompatible del paquete `openai` (pre v4) | Verificar `npm list openai` muestra v6.x+. Si no, ejecutar `npm install openai@latest` |
| La barra de progreso se queda en 45% mucho tiempo | La extracción de FastAPI está tardando (PDF grande o red lenta) | Es normal para PDFs de 100+ páginas. Espera hasta 30 segundos. Si pasa de 30s, se mostrará error de timeout |
| El botón queda habilitado después de generar el plan | Se llamó `setIsLoading(false)` en el flujo exitoso antes del `router.push` | Mantén `isLoading=true` hasta navegar; solo usa `setIsLoading(false)` dentro del `catch` |
| `Error 504: OpenAI no respondió en 45 segundos` | Timeout al generar el plan (red lenta o modelo sobrecargado) | Reintentar. Si persiste, verificar conexión a internet. Considerar usar un modelo más rápido (`gpt-4o-mini`) |
| El plan tiene menos sesiones de las esperadas | El modelo no generó sesiones para todos los días | La validación detecta esto y retorna error. Reintenta la generación |
| `Error 429: insufficient_quota` con GPT-5 | La cuenta de OpenAI no tiene créditos o plan de pago activo | Verificar [platform.openai.com/billing](https://platform.openai.com/billing). Si solo tienes Gemini funcional, selecciona "Gemini 2.5 Flash" en el selector de modelo |
| `Error 401: API key inválida para gpt-5` | `OPENAI_API_KEY` no está configurada o está expirada en `.env.local` | Verificar que `.env.local` tiene `OPENAI_API_KEY=sk-proj-...`. Los cambios en `.env.local` requieren reiniciar `npm run dev` |
| `Error 500: GEMINI_API_KEY no está definida` | Falta configurar la key de Gemini en `.env.local` | Agregar `GEMINI_API_KEY=AIza...` y `GEMINI_OPENAI_BASE_URL=https://generativelanguage...` en `.env.local`. Reiniciar `npm run dev` |
| Gemini retorna JSON truncado (incompleto) | `max_tokens: 4000` impuesto en el test pero **no en la ruta real** | La ruta `/api/plan/generate` **no tiene cap de tokens**. Si ves truncamiento, es un bug del test, no del endpoint. El endpoint real deja que el modelo termine naturalmente con `finish_reason: "stop"` |
| `gemini-2.5-pro` da 429 pero `gemini-2.5-flash` funciona | Pro tiene rate limits más estrictos que Flash | Usar `gemini-2.5-flash` como default. En pruebas reales con 63 tópicos, Flash produjo planes idénticamente válidos que Pro (0 errores de validación ambos) |
| El redirect a `/plan` da 404 | La ruta `/plan` no existía en el diseño original de UP-04 | Se creó `app/(dashboard)/plan/page.tsx` + `_components/plan-preview.tsx`. Si persiste, reiniciar `npm run dev` para que Next.js registre la nueva ruta |
| `/plan` muestra "Plan temporal no encontrado" | El plan se guardó en `sessionStorage` de la pestaña original, pero se abrió en otra pestaña o se recargó después de cerrar | `sessionStorage` vive por pestaña. Volver a `/setup` y regenerar el plan. UP-05 resolverá esto con persistencia real en Supabase |
| `Error 502: temperature is not supported for this model` | GPT-5 rechaza el parámetro `temperature` en algunos escenarios | El código ya envía `temperature: 0.3` solo para Gemini. GPT-5 usa los defaults del proveedor |
| El PDF de 63 tópicos tarda más de 45s en generar | Planes con 63 tópicos producen ~18,000 tokens y pueden tomar hasta 50s en Flash para 7 días | El timeout ahora **escala con `objective_days`**: 45s base + 6s/día extra para Gemini (cap 120s). 7 días → 45s, 14 días → 84s, 30 días → 120s. GPT-5 escala igual con 90s base + 8s/día (cap 180s) |

| `gemini-2.5-flash` falla con `Request was aborted` al generar 14+ días (28+ sesiones) | Timeout fijo de 45s insuficiente para planes largos | El timeout ahora escala con `objective_days` vía `calcModelTimeout()`. 14 días → 84s. Verifica que `createPlanModelRuntime(model_provider, objective_days)` recibe ambos argumentos. Reinicia `npm run dev` |
...
1. ✅ **Creaste `lib/openai.ts`** — Cliente OpenAI centralizado con validación de API key, modelo configurable por variable de entorno, timeout de 60s, y reintentos automáticos.

2. ✅ **Diseñaste el prompt de generación de plan** — System prompt con reglas detalladas (orden K, agrupación temática, duración por sesión) y user prompt compacto con `MAX_TOPICS_IN_PROMPT` + `text_preview` para controlar tokens.

3. ✅ **Creaste `app/api/plan/generate/route.ts`** — Route Handler completo con `runtime = "nodejs"`, `parseBody()`, autenticación → validación → fetch tópicos → ownership check → llamada al proveedor LLM → validación del plan → respuesta.

4. ✅ **Extendiste a multi-proveedor** — `createPlanModelRuntime()` selecciona el cliente correcto según el modelo: Gemini usa `GEMINI_API_KEY` contra endpoint OpenAI-compatible; GPT-5 usa `OPENAI_API_KEY` contra api.openai.com. Gemini envía `temperature: 0.3`; GPT-5 usa defaults.

5. ✅ **Modificaste `setup/page.tsx`** — Flujo de 3 etapas (upload → extract → generate) con barra de progreso de 4 niveles, visualización del resultado del plan y redirect final a `/plan?document_id=...`. Guarda el plan en `sessionStorage` como puente temporal antes de UP-05.

6. ✅ **Agregaste selector de modelo en `study-config.tsx`** — El usuario elige entre "Gemini 2.5 Flash" y "GPT-5" desde la UI. El valor viaja como `model_provider` al backend sin exponer API keys.

7. ✅ **Creaste `app/(dashboard)/plan/`** — Página `/plan` con `PlanPreview` que lee de `sessionStorage` y muestra el plan generado con métricas, resumen y tarjetas de sesiones, mientras UP-05 implementa la persistencia definitiva.

8. ✅ **Comparaste Flash vs Pro** — Con 63 tópicos reales, ambos modelos produjeron planes válidos sin errores de validación. Flash (18,646 tokens, 46s) y Pro (14,154 tokens, 43s) son equivalentes para generación de planes.

9. ✅ **Timeout escalable con `objective_days`** — `calcModelTimeout()` reemplaza los timeouts fijos. Gemini: 45s base + 6s/día (cap 120s). GPT-5: 90s base + 8s/día (cap 180s). Planes de 14 días (28 sesiones) ahora tienen 84s en vez de fallar a los 45s.

### Validar tu progreso

Para verificar que completaste UP-04 correctamente, puedes pedirle al agente:

```
Valida mi progreso en UP-04
```

Esto activará el skill de validación que revisará que todos los archivos existen, que las funciones están correctamente definidas, y que el flujo funciona.

### Próxima Guía: UP-05 — Guardar plan en Supabase

En la siguiente guía, completaremos el ciclo de persistencia:

- **Guardar en `study_plans`**: Insertar el plan completo (`plan_json`) con fechas y status "active"
- **Guardar en `sessions`**: Crear una fila por cada sesión del plan (14 filas para un plan de 7 días)
- **Guardar en `topic_progress`**: Crear una fila por cada tópico con status "pending" (40-63 filas)
- **Transacción atómica**: Si algo falla, no queda a medias — todo o nada
- **Retornar `plan_id`** al frontend para redirigir a la visualización del plan

UP-05 conectará lo que generamos aquí (el JSON del plan) con la base de datos real, completando el puente entre la IA y los datos persistentes. Además, eliminará la dependencia de `sessionStorage` que usamos como puente temporal en `/plan`.
...
# === OpenAI / GPT-5 ===
OPENAI_API_KEY=sk-proj-...        # Key de OpenAI para GPT-5

# === Google Gemini ===
GEMINI_API_KEY=AIza...            # Key de Google AI Studio
...
> **⚠️ IMPORTANTE:** Ya no existe `OPENAI_MODEL` ni `OPENAI_BASE_URL`. El modelo se elige desde la UI (`model_provider`) y el backend crea el cliente correcto dinámicamente.

#### Factory de Clientes: `createPlanModelRuntime()`

```typescript
// app/api/plan/generate/route.ts — Timeout escalable (post-UP-05)
// Escala con objective_days: más días = más sesiones = más tokens.
function calcModelTimeout(
...
function createPlanModelRuntime(
  provider: PlanModelProvider,
  objectiveDays: number,
): PlanModelRuntime {
  ...
  // Gemini: usa el endpoint OpenAI-compatible de Google
  return {
    provider,
...
|---|---|
| **Gemini usa `GEMINI_API_KEY` con fallback a `OPENAI_API_KEY`** | El proyecto usaba `OPENAI_API_KEY` para Gemini antes de tener variables separadas. El fallback mantiene compatibilidad |
| **GPT-5 omite `temperature`** | OpenAI rechaza `temperature` en modelos modernos cuando se usa `response_format: json_object` |
| **Timeouts escalables con `objective_days`** | Gemini: 45s base + 6s/día (cap 120s). GPT-5: 90s base + 8s/día (cap 180s). Cada día extra agrega 2 sesiones → más tokens de salida → más tiempo necesario. 14 días = 84s Gemini, 112s GPT-5 |
| **`max_tokens` no se configura** | El endpoint no impone límite de tokens de salida. Dejar que el modelo termine naturalmente con `finish_reason: "stop"` evita JSON truncado |
