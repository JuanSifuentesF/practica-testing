...
sequenceDiagram
    ...
    PG->>PG: ✅ Validar JSON:<br/>• ¿tiene sessions?<br/>• ¿sessions.length > 0?<br/>• ¿cada sesión tiene topic_codes válidos?<br/>• ¿todos los topic_codes existen en topics_json?
...
2. **Duración**: Cada sesión dura 90 minutos (45 min de teoría + 45 min de quiz).
...
function buildUserPrompt(
  ...
  // Formateamos cada tópico como una línea con su código, nivel K
  // y nombre para que el modelo tenga contexto claro.
  const topicsList = Object.entries(topics)
    .sort(([codeA], [codeB]) => codeA.localeCompare(codeB))
    .map(([code, entry]: [string, TopicEntry]) => {
      const name = entry.name || "Sin nombre";
      return `- ${code} (${entry.level_k}): ${name}`;
    })
    .join("\n");
...
## LISTA DE TÓPICOS (${totalTopics} tópicos)
...
function validateGeneratedPlan(
  ...
  // ─── 2. Verificar campos de cada sesión ─────────────────────
  ...
  // ─── 5. Verificar que no se omitieron tópicos ──────────────
  const missingTopics = Array.from(originalCodes).filter(
    (code) => !codeSet.has(code)
  );

  if (missingTopics.length > 0) {
    // Esto es un warning, no un error crítico. GPT puede omitir
    // algunos tópicos si hay muchos. Lo logueamos pero no fallamos.
    console.warn(
      `[Plan] ⚠️ ${missingTopics.length} tópicos omitidos del plan: ${missingTopics.join(", ")}`
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    if (!config || typeof config !== "object") {
      return NextResponse.json(
        { error: "config es requerido y debe ser un objeto con objective_days, morning_time y night_time." },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        {
          error: `objective_days debe ser un número entre ${MIN_DAYS} y ${MAX_DAYS}.`,
        },
        { status: 400 }
      );
    }

    if (!morning_time || !/^\d{2}:\d{2}$/.test(morning_time)) {
      return NextResponse.json(
        { error: 'morning_time es requerido y debe tener formato "HH:MM".' },
        { status: 400 }
      );
    }

    if (!night_time || !/^\d{2}:\d{2}$/.test(night_time)) {
      return NextResponse.json(
        { error: 'night_time es requerido y debe tener formato "HH:MM".' },
        { status: 400 }
      );
    }
    ...
    // estimated_end_date: hoy + objective_days
    const today = new Date();
    const startDate = today.toISOString().split("T")[0]; // "YYYY-MM-DD"

    const endDate = new Date(today);
    endDate.setDate(endDate.getDate() + objective_days);
...
- [ ] Valida la respuesta: estructura, topic_codes, duplicados
...
Actualmente, después de la extracción exitosa, la página redirige al dashboard con un `setTimeout`. Ahora necesitamos agregar una **tercera etapa**: la generación del plan.
...
export default function SetupPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Damos más tiempo al usuario para ver el resumen del plan.
      // En UP-05/UP-06, esto cambiará para navegar a la página
      // de visualización del plan generado.
      setTimeout(() => {
...
- [ ] El error handling restaura `isLoading` a `false`
...
4. **Verificación visual al completar:**
   ...
   - Deberías ver la fecha de inicio (hoy) y fin estimado (hoy + 7 días)
   - Deberías ver el modelo usado (gpt-4o-mini)
   - Después de ~3 segundos, redirige al dashboard
...
| Body inválido | Enviar POST sin config | Error 400: "config es requerido" |

### Checklist de Entregables Completo

- [ ] `frontend/lib/openai.ts` creado — exporta `openai` y `OPENAI_MODEL`
- [ ] `frontend/app/api/plan/generate/route.ts` creado — Route Handler completo
...
2. ✅ **Diseñaste el prompt de generación de plan** — System prompt con reglas detalladas (orden K, agrupación temática, duración por sesión) y user prompt con los datos del estudiante (tópicos, días, horarios).

3. ✅ **Creaste `app/api/plan/generate/route.ts`** — Route Handler completo con 7 fases: autenticación → validación → fetch tópicos → ownership check → llamada a OpenAI → validación del plan → respuesta.

4. ✅ **Modificaste `setup/page.tsx`** — Flujo de 3 etapas (upload → extract → generate) con barra de progreso de 4 niveles y visualización del resultado del plan (sesiones, fechas, modelo).

5. ✅ **Validación defensiva** — El plan generado se valida contra los tópicos originales: verificas que no hay codes inventados, no hay duplicados, y la estructura es correcta.
