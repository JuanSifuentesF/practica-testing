...
| El PDF de 63 tópicos tarda más de 45s en generar | Planes con 63 tópicos producen ~18,000 tokens y pueden tomar hasta 50s en Flash, 45s en Pro | El timeout del endpoint es de 45s para Gemini y 90s para GPT-5. Si falla por timeout en Gemini, reintentar o usar GPT-5 |
...
```typescript
// app/api/plan/generate/route.ts — Función extracto conceptual

function createPlanModelRuntime(provider: PlanModelProvider): PlanModelRuntime {
  if (provider === "gpt-5") {
    return {
      provider,
      model: "gpt-5",
      timeoutMs: 90_000,       // GPT puede tardar más
      client: new OpenAI({
...
| **Timeouts diferentes** | GPT-5 es más lento (90s). Gemini es más rápido (45s). Ambos con 15s extra en el cliente para dar margen al AbortController |
