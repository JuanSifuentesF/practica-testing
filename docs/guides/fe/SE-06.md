...
> **Fecha:** 29 junio 2026
> **Addendum correctivo vigente (20/07/2026):** el body contiene únicamente
> `attempt_id` y selecciones `{ question_id, user_answer }`. El servidor recupera
> answer key, tópico y nivel K del snapshot `private`; el RPC claimed valida el
> lease y finaliza grading + adaptación en una transacción. Los contratos que
> aceptan `correct`, `topic_code` o `level_k` del navegador son históricos.
> Settings conserva la autoridad de proveedor/modelo; JSON cualitativo inválido
> no habilita fallback facturable a otro proveedor/modelo.
...
## Addendum Correctivo: Cascada Best-Effort del Análisis Cualitativo

La evaluación de sesión mantiene el score, `action_taken` y la persistencia determinísticos como fuente de verdad. Su análisis cualitativo ahora usa `frontend/lib/ai/model-cascade.ts`: intenta `GEMINI_SESSION_EVALUATE_MODEL`, candidatos Gemini y `OPENAI_SESSION_EVALUATE_MODEL`/`gpt-4o-mini` antes de aplicar el feedback determinístico.

Cada candidato usa `AbortController` con `EVALUATE_TIMEOUT_MS`. Un timeout, fallo de proveedor o JSON inválido no interrumpe la evaluación del quiz y no expone contenido LLM en logs.
