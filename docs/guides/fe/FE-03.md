...
- [ ] `.env.local` contiene `NEXT_PUBLIC_SUPABASE_URL` con valor real
- [ ] `.env.local` contiene `NEXT_PUBLIC_SUPABASE_ANON_KEY` con valor real
- [ ] `npm run dev` arranca sin errores y detecta `.env.local`
...
- [ ] Archivo `frontend/middleware.ts` creado en la raíz del frontend
- [ ] El archivo NO está dentro de `app/`
- [ ] El matcher excluye archivos estáticos, imágenes y `/auth/callback`
- [ ] Se usa `getUser()` (no `getSession()`) para la verificación
...
- [ ] Archivo `frontend/app/auth/callback/route.ts` creado
- [ ] La ruta es `app/auth/callback/` (sin paréntesis), no `app/(auth)/callback/`
- [ ] El handler lee el parámetro `code` de la URL
- [ ] Usa `exchangeCodeForSession()` para convertir código en sesión
- [ ] Redirige al dashboard en éxito, al login en error
...
- [ ] Archivo `frontend/app/(auth)/layout.tsx` creado
- [ ] Es un Server Component (sin `'use client'`)
- [ ] Incluye fondo decorativo con gradiente radial
- [ ] El contenedor tiene `max-w-md` para formularios legibles
- [ ] Incluye logo "ISTQB Agent" con link al home
- [ ] La tarjeta del formulario tiene efecto glassmorphism
...
- [ ] Archivo `frontend/app/(auth)/login/page.tsx` creado
- [ ] Es un Client Component (`'use client'` en la primera línea)
- [ ] Usa `signInWithPassword()` de Supabase
- [ ] Tiene manejo de errores con renderizado condicional
- [ ] Tiene estado de `loading` que desactiva el botón
- [ ] Llama a `router.push()` + `router.refresh()` en éxito
- [ ] Incluye link de navegación a `/register`
...
- [ ] Archivo `frontend/app/(auth)/register/page.tsx` creado
- [ ] Es un Client Component (`'use client'`)
- [ ] Tiene campo adicional `Nombre Completo` con estado `fullName`
- [ ] `signUp()` envía `options.data.full_name` (guión bajo, no camelCase)
- [ ] Tiene manejo de errores y estado de loading
- [ ] Incluye link de navegación a `/login`
...
- [ ] Archivo `frontend/app/(dashboard)/dashboard/page.tsx` creado
- [ ] Es un Client Component (`'use client'`)
- [ ] Muestra un mensaje de bienvenida al dashboard
- [ ] Tiene indicadores de estado (middleware, sesión, cookies)
- [ ] Implementa `handleLogout` con `signOut()` + `router.push()` + `router.refresh()`
- [ ] Usa `variant="destructive"` en el botón de logout
...
- [ ] `npx tsc --noEmit` se ejecuta sin errores
- [ ] Los 6 archivos creados existen en las ubicaciones correctas
...
**Resultado esperado:**
- [ ] El middleware te intercepta **instantáneamente** (no ves la página de dashboard)
- [ ] La URL en la barra de direcciones cambia automáticamente a `http://localhost:3000/login`
- [ ] Ves el formulario de inicio de sesión centrado con el fondo oscuro
- [ ] El logo "ISTQB Agent" está visible arriba del formulario

### Checkpoint 3: Página de Login

Navega a `http://localhost:3000/login`:

- [ ] Ves el formulario centrado con el título "Iniciar Sesión"
- [ ] Hay un campo "Correo Electrónico" con placeholder "tu@email.com"
- [ ] Hay un campo "Contraseña" (tipo password, texto oculto)
- [ ] Hay un botón verde "Entrar" de ancho completo
- [ ] Debajo del formulario hay un link "¿No tienes cuenta? Regístrate aquí"
- [ ] El fondo tiene un gradiente sutil verdoso
- [ ] La tarjeta del formulario tiene efecto de vidrio (glassmorphism)

### Checkpoint 4: Página de Registro

Navega a `http://localhost:3000/register`:

- [ ] Ves el formulario centrado con el título "Crear Cuenta"
- [ ] Hay un campo extra: "Nombre Completo" con placeholder "Ej. Juan Pérez"
- [ ] Hay un campo "Correo Electrónico"
- [ ] Hay un campo "Contraseña (Mín. 6 caracteres)"
- [ ] Hay un botón azul "Registrarse" de ancho completo
- [ ] Debajo hay un link "¿Ya tienes cuenta? Inicia sesión"
...
**Resultado esperado:**
- [ ] El botón cambia a "Creando cuenta..." temporalmente
- [ ] Eres redirigido automáticamente a `http://localhost:3000/dashboard`
- [ ] Ves la página "Dashboard Seguro" con el mensaje de felicitación
- [ ] Los badges muestran "✅ Middleware: activo", "✅ Sesión: válida", "✅ Cookies: sincronizadas"

3. **Verificación en Supabase Studio** (opcional pero recomendado):
   - Abre Supabase Dashboard → **Authentication** → **Users**
   - [ ] Ves el usuario `test1@test.com` creado
   - Navega a **Table Editor** → `user_profiles`
   - [ ] Ves una fila con el mismo ID y `full_name = 'Juan Test'` (¡el trigger de DB-05 funcionó!)
...
**Resultado esperado:**
- [ ] El middleware detecta que ya tienes sesión
- [ ] Te redirige automáticamente de vuelta a `http://localhost:3000/dashboard`
- [ ] No puedes acceder a `/login` ni a `/register` mientras tengas sesión
...
**Resultado esperado:**
- [ ] Eres redirigido a `http://localhost:3000/login`
- [ ] Si intentas ir a `http://localhost:3000/dashboard`, eres redirigido a `/login`
- [ ] El ciclo de protección se reinicia completamente
