 ...
 * pasado a `content` o al invocar la función directamente.
 * Usamos `any` porque los genéricos del Tooltip (ValueType/NameType)
 * no se propagan correctamente a tipos personalizados en JSX con
 * Recharts 3.x. Esto es una limitación conocida de la librería
 * y la práctica recomendada por la comunidad.
 *
 * DISEÑO:
    ...
    <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
      ...
      <ResponsiveContainer width="100%" height={300}>
        ...
        >
          ...
          {/* ─── Tooltip personalizado ─── */}
          {/* content={CustomTooltip} pasa la función directamente.
               Recharts 3.x la invoca inyectando active + payload.
               Usar la referencia directa evita conflictos de tipos
               que ocurren al pasar un JSX element (<CustomTooltip />). */}
          <Tooltip
...
| 9 | `TS2339: Property 'payload' does not exist on type 'TooltipProps<number, string>'` | Recharts 3.x movió `payload`, `active` y `coordinate` de `TooltipProps` a `TooltipContentProps`. Además, los genéricos del `<Tooltip>` no se propagan a tipos personalizados en JSX | **Dos cambios necesarios:** (1) NO importar `TooltipProps` de Recharts; usar `any` en la firma de `CustomTooltip({ active, payload }: any)`. (2) Pasar la función directamente: `content={CustomTooltip}` en lugar de `content={<CustomTooltip />}`. Esto evita conflictos de `ContentType<ValueType, NameType>` vs `ContentType<number, string>` |
