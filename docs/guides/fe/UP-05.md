...
| ✏️ **MODIFICAR** | `app/(dashboard)/plan/page.tsx` | Aceptar `plan_id` como query param para cargar desde DB |
...
Buscar el bloque que está justo ANTES del bloque que acabamos de reemplazar:
```typescript
      // ─── Guardar resultado del plan ─────────────────────────
      setPlanResult({
        total_sessions: planData.total_sessions,
...
**Eliminar** este bloque por completo (ya no se necesita aquí porque lo movimos a después del save).
...
**Cambios:** Aceptar `plan_id` como query parameter en lugar de (o además de) `document_id`.
...
export default async function PlanPage({ searchParams }: PlanPageProps) {
  ...
  const planId = Array.isArray(rawPlanId) ? rawPlanId[0] : rawPlanId || null;
...
5. **Modificamos** `plan/page.tsx` para aceptar `plan_id` como parámetro de URL, con backward compatibility para `document_id`.
