...
| El chart aparece en blanco | `ResponsiveContainer` no tiene altura disponible | Asegura que el contenedor tenga `h-[360px] w-full min-w-0` y que `ResponsiveContainer` use `width="100%" height="100%"` |
| `The width(-1) and height(-1) of chart should be greater than 0` | CSS Grid colapsa min-width a 0 o el wrapper no tiene ancho explícito | Agregá `min-w-0` al `div` que envuelve al `ResponsiveContainer` y a la grilla padre (`grid ... min-w-0`). En ScoreChart (DA-02), el wrapper exterior necesita `w-full` |
