...
sequenceDiagram
    ...
    EA->>FA: POST /extract-pdf (FormData con PDF)
...
graph TD
    ...
    G -->|"POST PDF binario"| J["FastAPI /extract-pdf"]
...
// FastAPI /extract-pdf, recibe los tópicos estructurados, y los
...
export async function POST(request: Request) {
  try {
    ...
    try {
      fastApiResponse = await fetch(`${FASTAPI_URL}/extract-pdf`, {
        method: "POST",
...
| `Error: fetch failed` en el servidor | Node.js no puede resolver la URL de FastAPI | Si usas localhost, asegúrate de que no hay conflicto de puertos. Prueba con `http://127.0.0.1:8000` en lugar de `localhost` |
...
1. **Creaste** `app/api/extract/route.ts` — un Route Handler que:
   ...
   - Envía el PDF a FastAPI `/extract-pdf` con timeout de 30s.
