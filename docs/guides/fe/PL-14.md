...
> **Nota de cierre documental:** las 20 casillas vacías bajo “Entregable del
> Paso” son plantillas históricas anteriores a la validación. La autoridad es la
> checklist final 18/18 de la Sección 6.
...
PL-14 cerró un único delta coherente:

1. agregó “Práctica” a las navegaciones desktop y mobile;
2. mantuvo el enlace activo en todo `/practice/*`;
3. migró `middleware.ts` a `proxy.ts` según Next.js `16.2.9`;
4. corrigió la brecha histórica de `/plan` y protegió `/practice`;
5. preservó cookies durante respuestas normales y redirects;
6. mantuvo el layout, Route Handlers y RLS como defensa en profundidad;
7. eliminó el warning de convención deprecada con `tsc` y build verificables.

La checklist final 18/18 conserva la evidencia del cierre. El siguiente
milestone histórico fue **AI-01**; para el gate vigente debe consultarse el
roadmap, porque auditorías posteriores pueden reabrir dependencias.
