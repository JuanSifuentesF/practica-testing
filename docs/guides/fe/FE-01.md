...
OPENAI_API_KEY=sk-<REDACTED_ROTATE_IF_REAL>
...
#### C.3.1 — Configuración CRÍTICA de `.gitignore` para Frontend

Para que Git pueda rastrear tu código de frontend y a la vez proteger tus secretos, sigue estos pasos:

1. **Edita el `.gitignore` de la RAIZ (`practica-testing/.gitignore`):**
   Asegúrate de que la línea `frontend/` **NO existe** (o coméntala añadiendo `#` al inicio). Si Git está ignorando toda la carpeta `frontend/`, no podrás subir tu código.

2. **Crea un `.gitignore` ESPECÍFICO para el frontend (`frontend/.gitignore`):**
   Crea este archivo para proteger tus dependencias locales y secrets dentro de la carpeta del frontend:
...
**Salida esperada (puede variar según la fecha — las versiones se actualizan constantemente):**
```
added 129 packages, removed 3 packages, and audited 486 packages in 37s
...
> Las vulnerabilidades moderadas son típicas en proyectos nuevos y no afectan el desarrollo local. Puedes ignorarlas o ejecutar `npm audit fix --force` si prefieres.
...
**Salida esperada (las versiones exactas pueden variar según el momento de la instalación):**
```
...
|---|---|---|
| Component library | **Radix** | Librería headless UI más completa (modales, selects, dropdowns). La alternativa "Base" es más liviana pero con menos componentes |
| Preset | **Luma** | Viene con Lucide icons + Inter font, diseño limpio y bien mantenido. Es el preset recomendado por defecto |
...
**Salida esperada (el contenido exacto depende de la versión de shadcn):**
```tsx
...
>
> **Novedades de shadcn v4:** El Button ahora incluye variantes de tamaño `xs`, `icon-xs`, `icon-sm`, `icon-lg`, usa `radix-ui` unificado en lugar de `@radix-ui/react-slot`, y utiliza `data-slot`/`data-variant` para estilos basados en atributos.
...
**Salida esperada (las versiones exactas pueden variar):**
```
...
> Nota: En shadcn v4, las dependencias de Radix se instalan bajo el paquete unificado `radix-ui` (no como `@radix-ui/react-slot` individual). Esto es normal y correcto.
...
- ✅ Dependencias de shadcn instaladas (`clsx`, `tailwind-merge`, `class-variance-authority`, `lucide-react`, `radix-ui`)
...
- [x] La terminal muestra `✓ Ready in X.Xs`
- [x] La terminal muestra `Environments: .env.local`

### Checkpoint 2: Página de bienvenida

Abre **http://localhost:3000** en el navegador:

- [x] Se muestra la página con fondo oscuro degradado
- [x] El título "ISTQB Study Agent" está visible con gradiente azul-verde
- [x] Se ven los 6 badges de stack tecnológico
- [x] El botón "🚀 Comenzar a estudiar" está visible con estilo shadcn/ui
- [x] El botón "📖 Ver documentación" con estilo outline está visible

### Checkpoint 3: Interactividad del Button

- [x] Al hacer click en "🚀 Comenzar a estudiar", el texto cambia a "✅ Clicks: 1"
- [x] Cada click subsiguiente incrementa el contador
- [x] Aparece el mensaje verde "¡shadcn/ui Button funciona!"
- [x] Después de 5+ clicks, el mensaje agrega "¡Estás listo para construir la app!"
...
```
- [x] Todas las 6 variables existen con valores reales
...
```
- [x] Retorna `True`
- [x] El archivo contiene los nombres de las variables pero NO valores reales
...
Get-ChildItem -Path ".next" -Recurse -Include "*.js" | Select-String -Pattern "OPENAI_API_KEY" -List
```
- [x] El build completa sin errores
- [x] NINGÚN resultado para `SUPABASE_SERVICE_ROLE_KEY`
- [x] NINGÚN resultado para `OPENAI_API_KEY`
...
> **Nota:** Tailwind CSS v4 no usa `tailwind.config.ts`. La configuración se maneja directamente en `globals.css` con la directiva `@import "tailwindcss"`. Si tu versión de Next.js generó un `tailwind.config.ts`, también es válido.
