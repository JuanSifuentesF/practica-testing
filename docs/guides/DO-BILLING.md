...
Usa el token nuevo generado en DigitalOcean, pero no lo guardes dentro de esta guía.
Pégalo solamente en la sesión actual de PowerShell:

```powershell
$env:DO_TOKEN = "PEGA_AQUI_TU_NUEVO_TOKEN_SOLO_EN_TU_TERMINAL"
```

> ⚠️ Esto solo vive en la sesión actual de PowerShell. Al cerrar la terminal, desaparece.
> No pegues tokens reales en archivos del repositorio, aunque `docs/` esté ignorado.
...
> Recomendado: si usas `doctl auth init`, pega el token cuando la CLI lo pida.
> No lo escribas previamente en archivos `.md`, `.env` versionados ni scripts.
